...
module.exports = {
  ...
  async _proceedPrompt(message, options) {
    if (options.yes) {
      return;
    }
  ...
  async _promptIfWorkingTreeDirty(options) {
    ...
    await this._proceedPrompt('Your working tree contains staged, but uncommitted changes that will be added to the release commit', options);
  },
  ...
  async _promptToCreateGitTag(options) {
    ...
    await this._proceedPrompt(message, options);
  },
